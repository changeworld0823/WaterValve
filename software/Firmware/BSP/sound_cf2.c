...
const static Melody range_slow = {.bpm = 120, .delay = 1, .notes = {{C4, H}, {D4, H}, {E4, H}, {F4, H}, {G4, H}, {A4, H}, {B4, H}, REPEAT}};
const static Melody range_fast = {.bpm = 120, .delay = 1, .notes = {{C4, S}, {D4, S}, {E4, S}, {F4, S}, {G4, S}, {A4, S}, {B4, S}, REPEAT}};
const static Melody startup = {.bpm = 120, .delay = 1, .notes = {{C6, S}, {C6, S}, STOP}};
const static Melody calibrated = {.bpm = 120, .delay = 1, .notes = {{C4, S}, {E4, S}, {G4, S}, {C5, E}, STOP}};
const static Melody chg_done = {.bpm = 120, .delay = 1, .notes = {{D4, Q}, {A4, Q}, STOP}};
const static Melody lowbatt = {.bpm = 120, .delay = 1, .notes = {{D4, E}, {A4, E}, {D4, E}, REPEAT}};
const static Melody usb_disconnect = {.bpm = 120, .delay = 1, .notes = {{C4, E}, STOP}};
const static Melody usb_connect = {.bpm = 120, .delay = 1, .notes = {{A4, E}, STOP}};
const static Melody factory_test = {.bpm = 120, .delay = 1, .notes = {{A1, Q}, {OFF, S}, {A2, Q}, {OFF, S}, STOP}};
const static Melody starwars = {.bpm = 120, .delay = 1, .notes = {{A3, Q}, {A3, Q}, {A3, Q}, {F3, ES}, {C4, S},
    ...
    REPEAT}};
const static Melody valkyries = {.bpm = 140, .delay = 1, .notes = {{Gb3, Q}, {B3, Q},
    {Gb3, S}, {B3, E},  {D4, Q}, {B3, Q}, {D4, Q}, {B3, S}, {D4, E}, {Gb4, Q},  
    {D4, Q}, {Gb4, Q}, {D4, S}, {Gb4, E}, {A4, Q}, {A3, Q}, {D4, Q}, {A3, S},  
    {D4, E}, {Gb4, H}, 
    REPEAT}};
const static Melody button = {.bpm = 240, .delay = 1, .notes = {{Gb5, S},
    STOP}};

const static Melody target_stop = {.bpm = 240, .delay = 1, .notes = {{Bb3, S},{Bb3, S},
    STOP}};

const static Melody over_current = {.bpm = 240, .delay = 1, .notes = {{F3, ES},{F3, ES},
    STOP}};

const static Melody target_do_diff = {.bpm = 120, .delay = 1, .notes = {{C5, E}, {G4, S}, {E4, S}, {C4, S}, STOP}};
...
static EffectCall effects[] = {
    ...
    [SND_BUTTON]  = {.call = &melodyplayer, .melody = &button},
...
void soundInit(void)
{
  ...
  extern void buzzDeckInit(DeckInfo *info);
  buzzDeckInit(0);
  
  neffect = sizeof(effects) / sizeof(effects[0]) - 1;

  timer = xTimerCreate("SoundTimer", pdMS_TO_TICKS(10), pdTRUE, NULL, soundTimer);
